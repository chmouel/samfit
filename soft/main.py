...
def parse_args():
    ...
    parser.add_argument(
        "--tp-token",
        default=os.environ.get("TP_TOKEN"),
        type=str,
        help="Trainingpeaks password",
    )
    ...
    tp_add_cadence_to_library = mainparser.add_parser(
        "tp_add_cadence_to_library", help="Add a cadence to all library workouts"
    )
    ...
    tp_get_all_workouts = mainparser.add_parser(
        "tp_get_all_workouts", help="Get all TrainingPeaks workouts id to a JSON file"
    )
    ...
    tp_import_plan = mainparser.add_parser(
        "tp_import_plan", help="Import our own generated plan starting from a date."
    )
    ...
    stuff_to_do = mainparser.add_parser(
        "stuff_to_do", help="Do some stuff on tp calendar."
    )
